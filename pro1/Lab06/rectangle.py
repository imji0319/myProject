...
def draw_rectangle(x,y,w,h,pensize=1, edge_color='black', fill_color = ''):
    ...
    mypen.forward(w); mypen.left(90)
    mypen.forward(h); mypen.left(90)
    mypen.forward(w); mypen.left(90)
    mypen.forward(h)
    
    mypen.end_fill()

def draw_board(tile_pos,tile_size,tile_num,width=1): 
    # 흑백타일 만들기
...
def gem_find_randomwalk(tile_pos,size,n):
    ...
    images=['image/right_open.gif' ,'image/right_close.gif' ,'image/left_open.gif','image/left_close.gif','image/die.gif','image/up_open.gif','image/up_close.gif','image/down_open.gif','image/down_close.gif','image/diamond.gif']
...
def main():
    ...
    gem_find_randomwalk(tile,50,11)
